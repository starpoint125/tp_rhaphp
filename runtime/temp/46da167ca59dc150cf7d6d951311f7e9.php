<?php /*a:2:{s:63:"D:\wamp\www\rhaphp-master\themes/pc/admin/appstore\upgrade.html";i:1537234068;s:58:"D:\wamp\www\rhaphp-master\themes/pc/admin/common\base.html";i:1537234068;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<div style="padding:0px 10px;">
    <table class="layui-table" lay-even="" >
        ...
        <tr>
            <td><img src="<?php echo htmlentities($v['logo']); ?>" width="30"></td>
            <td><?php echo htmlentities($v['name']); ?></td>
            <td><?php echo htmlentities($v['addon']); ?></td>
            <td><?php echo htmlentities($v['author']); ?></td>
            <td><?php echo htmlentities($v['version']); ?></td>
            <td><?php echo htmlentities($v['new_version']); ?></td>
            <td><?php echo htmlentities(date("Y-m-d H:i",!is_numeric($v['update_time'])? strtotime($v['update_time']) : $v['update_time'])); ?></td>
            <td> <button onclick="upgrade('<?php echo htmlentities($v['app_id']); ?>','<?php echo htmlentities($v['addon']); ?>','<?php echo htmlentities($v['version']); ?>')" class="layui-btn layui-btn-xs">升级</button></td>
        </tr>
        <?php endforeach; endif; else: echo "" ;endif; ?>
        </tbody>
    </table>
            ...
            $.post(" ",{app_id:id,addon:addon,version:version},function (res) {
                if(res.errcode=='0'){
                    layer.msg(res.errmsg,{time:3000},function () {
                        $.post("<?php echo url('admin/app/install'); ?>",{'name':addon},function (res) {
                            if(res.status=='0'){
                                layer.alert(res.msg);
                            }
                            if(res.status=='1'){
