<?php /*a:2:{s:60:"D:\wamp\www\rhaphp-master\themes/pc/admin/miniapp\index.html";i:1537234068;s:58:"D:\wamp\www\rhaphp-master\themes/pc/admin/common\base.html";i:1537234068;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<div class="layui-tab">
    <ul class="layui-tab-title">
        <li <?php if($type == 'uninstall'): ?>class="layui-this"<?php endif; ?>> <a href="<?php echo url('index',['type'=>'uninstall']); ?>">我的应用</a></li>
        <li <?php if($type == 'notinstall'): ?>class="layui-this"<?php endif; ?>><a href="<?php echo url('index',['type'=>'notinstall']); ?>">安装|卸载</a></li>
    </ul>
</div>
<?php switch($type): case "uninstall": ?>
    <table class="layui-table" lay-even lay-skin="nob">
        ...
        <tr>
            ...
            <td width="90">
                <button class="layui-btn layui-btn-xs" onclick="closeApp('<?php echo htmlentities($vo['addon']); ?>')">停用</button>
...
<table class="layui-table" lay-even lay-skin="nob">
    ...
    <?php if(is_array($addons) || $addons instanceof \think\Collection || $addons instanceof \think\Paginator): $i = 0; $__LIST__ = $addons;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
    <tr >
        <td><img width="30" height="30" src="<?php echo getAddonLogo($vo['addon'],'miniapp'); ?>"></td>
        <td><?php echo htmlentities($vo['name']); ?></td>
        <td><?php echo htmlentities($vo['addon']); ?></td>
        <!--<td><?php echo htmlentities($vo['desc']); ?></td>-->
        <td><?php echo htmlentities($vo['version']); ?></td>
...
<script>
    ...
    function installApp(name) {
            ...
            $.post("<?php echo url('install'); ?>",{'name':name},function (res) {
                if(res.status=='0'){
                ...
                if(res.status=='1'){
                    layer.msg(res.msg,{time:1000},function () {
                        location.href="<?php echo url('index',['type'=>'notinstall']); ?>";
                    });
                }
    ...
    function upgrade(name) {
            $.post("<?php echo url('upgrade'); ?>",{'name':name},function (res) {
                if(res.status=='0'){
                    layer.alert(res.msg);
                }
                if(res.status=='1'){
    ...
    function closeApp(name) {
            $.post("<?php echo url('close'); ?>",{'name':name},function (res) {
                if(res.status=='0'){
                    layer.alert(res.msg);
                }
                if(res.status=='1'){
                    layer.msg(res.msg,{time:1000},function () {
                        location.href="<?php echo url('index',['type'=>'uninstall']); ?>";
                    });
                }
            })
    }
    ...
    function uninstallApp(name) {
        layer.confirm('你确定卸载（'+name+'）此应用吗？点击确定将会清空此应用所有数据以及文件', {
            ...
            $.post("<?php echo url('uninstall'); ?>",{'name':name},function (res) {
                if(res.status=='0'){
                    layer.close(index)
                    layer.alert(res.msg);
                }
    ...
    function wipeDataApp(name) {
        layer.confirm('你确定还（'+name+'）此应用吗？点击确定将会清空数据以及还原未安装状态', {
            ...
            $.post("<?php echo url('wipeData'); ?>",{'name':name},function (res) {
                if(res.status=='0'){
                    layer.close(index)
                    layer.alert(res.msg);
                }
